...
async def test_reconnect_gets_new_identity_and_local_state(workspace: Path):
    async with mcp_session(workspace) as first:
    ...
    async with mcp_session(workspace) as second:
        observed = await execute(
            ...
            "import json\n"
            "json.dumps({'id': ws.client.id, 'connection_id': ws.client.connection_id, "
            "'shared': shared_value, 'has_local': 'persisted' in ws.local})",
        )
    ...
    assert second_info["shared"] == 73
    assert not second_info["has_local"]


def _find_client_processes(workspace: Path) -> set[int]:
    candidates = []
    for entry in Path("/proc").iterdir():
        ...
        try:
            ...
            process_workspace = (entry / "cwd").resolve()
        except OSError, UnicodeDecodeError:
            continue
        if (
            b"-m" in command
            and b"mypr_mcp.cli" in command
            and b"serve" in command
            and process_workspace == workspace.resolve()
        ):
...
async def test_status_tracks_connections_unique_clients_and_abrupt_disconnect(workspace: Path):
    async with mcp_session(workspace) as first:
        ...
        async with mcp_session(workspace) as abrupt:
            ...
            new_processes = _find_client_processes(workspace) - existing_processes
            assert len(new_processes) == 1
            pid = new_processes.pop()
...
async def test_history_lists_and_gets_execution_and_task_owners(workspace: Path):
    async with mcp_session(workspace) as first:
        client_id = result_text(await execute(first, "ws.client.id")).strip(" '\n")
        ...
        records = await execute(
            first,
            f"import json\njson.dumps(await ws.history.list(client_id={client_id!r}))",
        )
        ...
        assert all(item["client_id"] == client_id for item in items)
        ...
        assert task_detail["client_id"] == client_id
...
async def test_history_logs_cursor_is_stable_and_does_not_duplicate(workspace: Path):
    async with mcp_session(workspace) as session:
        client_id = result_text(await execute(session, "ws.client.id")).strip(" '\n")
        await execute(session, "'first-log-event'")
        first = await execute(
            session,
            f"import json\njson.dumps(await ws.history.logs(client_id={client_id!r}, limit=100))",
        )
        ...
        second = await execute(
            session,
            "import json\n"
            f"json.dumps(await ws.history.logs(client_id={client_id!r}, "
    ...
    async with mcp_session(workspace) as first:
        ...
        async with mcp_session(workspace) as second:
            ...
            logs = json_output(
                await execute(
                    second,
                    f"await ws.history.logs(client_id={client_id!r}, limit=200)",
                )
            )
            outputs = [
                event
            ...
            assert "".join(event["data"]["text"] for event in outputs) == (
                f"live-owner:{client_id}\n"
            )
            assert all(event["client_id"] == client_id for event in outputs)
